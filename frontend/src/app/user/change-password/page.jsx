...
import { useChangePasswordMutation } from "@/lib/services/auth"; // ✅ ตรวจสอบว่ามี import
...
const ChangePassword = () => {
    ...
    const [changePassword] = useChangePasswordMutation(); // ✅ ตรวจสอบให้มั่นใจว่าตัวแปรนี้มีค่าจริง
        ...
        useFormik({
            ...
            onSubmit: async (values) => {
                try {
                    ...
                    setServerErrorMessage(err.data?.message || "Something went wrong");
                } finally {
                    setLoading(false);
                }
            },
    ...
    return (
        <div className="flex items-center justify-center h-screen bg-gray-100">
            <div className="w-full max-w-md p-8 bg-white rounded-lg shadow-lg">
                <h2 className="text-2xl font-bold mb-6 text-center">User Change Password</h2>
                ...
                {serverErrorMessage && <div className="text-sm text-red-500 mt-2">{serverErrorMessage}</div>}
                {serverSuccessMessage && <div className="text-sm text-green-500 mt-2">{serverSuccessMessage}</div>}
            </div>
        </div>
    );
};
